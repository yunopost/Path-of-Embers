...
func set_energy(current: int):
	## Energy has no maximum (Design ruling, 10 Sep 2026): it accumulates without a
	## ceiling. The floor at 0 is a deliberate rule, not an accident of the numbers —
	## energy is a resource, not a debt, so callers must never pass a negative value.
	if energy != current:
		energy = current
		energy_changed.emit()
