extends Node
## Deterministic screen-driven run attempts. Writes user://run-flow-regression.json.
## Death is a valid outcome; a stuck flow is a regression. Uses a disposable profile.
...
func run_tests():
	flow_only = "--flow-only" in OS.get_cmdline_user_args()
	print("FLOW MODE: ", "controlled progression (combat bypassed)" if flow_only else "ordinary timed policy")
	...
	for trial in range(3):
		...
		await settle()
		if flow_only:
			# The discard/gate contract is tested independently in release_blockers.
			# This mode isolates three-act navigation and reward state transitions.
		...
		for step in range(5000):
			...
			match screen:
				...
				"combat":
					...
					if flow_only:
						for enemy in cc.get_enemies():
							if enemy.stats.is_alive():
								enemy.stats.take_damage(100000, true)
						continue
				...
				"rewards":
					...
					if b.upgrade_count > 0:
						...
						for iid in RunState.get_upgradeable_instance_ids():
							if b.is_transcendence_upgrade:
								s.upgrade_flow_panel._on_card_widget_clicked(iid)
								await settle()
								for button in s.upgrade_flow_panel.upgrade_content.get_children():
	...
	f.close()
	policy.free()
	var flow_failed := false
	for report in reports:
		if report.outcome not in ["victory", "death"]:
			flow_failed = true
	get_tree().quit(1 if flow_failed else 0)
