extends RefCounted
class_name EffectType

## Enum-like class for effect types - provides type safety and avoids string typos
## Usage: EffectType.DAMAGE, EffectType.BLOCK, etc.

# Core effects
const DAMAGE = "damage"
const BLOCK = "block"
const HEAL = "heal"
const DRAW = "draw"

# Status effects
const VULNERABLE = "vulnerable"
const VULNERABLE_ALL_ENEMIES = "vulnerable_all_enemies"
const STRENGTH = "strength"
const DEXTERITY = "dexterity"
const FAITH = "faith"
const WEAKNESS = "weakness"

# Card manipulation
## The next card played gains Haste (0 ticks). Optional params:
## {conditional_draw_min_cost, conditional_draw_amount} -- if given, when the
## marked next card is actually played, draw conditional_draw_amount cards if
## that card's effective cost is >= conditional_draw_min_cost. Monster Hunter - Pursuit.
const GRANT_HASTE_NEXT_CARD = "grant_haste_next_card"
const ADD_CURSE_TO_HAND = "add_curse_to_hand"
const ADD_TEMPORARY_UPGRADE_TO_RANDOM_HAND_CARD = "add_temporary_upgrade_to_random_hand_card"

# Conditional effects
const CONDITIONAL_STRENGTH_IF_NO_DAMAGE = "conditional_strength_if_no_damage"
const DAMAGE_PER_CURSE = "damage_per_curse"
const DAMAGE_CONDITIONAL_ELITE = "damage_conditional_elite"

# Special effects
const BLOCK_ON_ENEMY_ACT = "block_on_enemy_act"
const DAMAGE_ON_BLOCK_GAIN = "damage_on_block_gain"

# Mechanist - Board Control
const DRAW_PER_TURN = "draw_per_turn"  # Power: draw N extra cards at start of each turn

# Mechanist - Legacy
const LEGACY_KILL_COUNTER = "legacy_kill_counter"  # Card permanently gains stats each time it lands a killing blow

# Legacy/placeholder
const APPLY_STATUS = "ApplyStatus"
const MODIFY_ENEMY_TIMER = "ModifyEnemyTimer"

# Revenant - Spite & Undying (stub)
const DAMAGE_SPITE = "damage_spite"  # Deals bonus damage scaled by missing HP: {base_amount, bonus_per_10_missing_hp}
const DRAW_IF_TOOK_DAMAGE = "draw_if_took_damage"  # Draw N cards if you took damage this combat: {amount}

# Tempest - Sequencing (stub)
const DAMAGE_SEQUENCING = "damage_sequencing"  # Bonus damage if last card played was an Attack: {base_amount, bonus_if_last_was_attack}

# Grove - Bloom & Regrowth (stub)
const BLOOM = "bloom"  # Add counters to a card; trigger effect when drawn at threshold: {counters, trigger_threshold, trigger_effect, trigger_amount}
const REGROWTH = "regrowth"  # Card returns to draw pile at end of turn instead of discarding: {}

# Sibyl - Foresight & Probability (stub)
const SCRY = "scry"  # Look at top N cards of draw pile and reorder them: {amount}
const DAMAGE_CONDITIONAL_TOP_CARD = "damage_conditional_top_card"  # More damage if top card is Attack, else less damage + block: {attack_amount, default_amount, default_block}

# Echo - Mirror & Resonance (stub)
const MIRROR = "mirror"  # Replay the last card played: {}
const RESONANCE_BLOCK = "resonance_block"  # Gain block; bonus block if last card was also a Skill: {base_amount, bonus_if_last_was_skill}

# Hollow - Conversion & Dominance (stub)
const BLOCK_TO_ENERGY = "block_to_energy"  # Convert all current Block into Energy at ratio: {block_per_energy}
# FORCE_END_TURN removed (Card-Clock Combat spec §5/§10.7): Dominance cards now carry
# the "Slow 3" keyword instead of forcing a full end-of-turn.

# Living Armor - Iron Tide
## Deal damage equal to current Block, divided by `divisor` (default 1) and rounded
## up: {divisor}. Never spends Block. Iron Tide uses divisor=2 ("half your Block").
const DAMAGE_EQUAL_TO_BLOCK = "damage_equal_to_block"

# ── Agility (Card-Clock signature cards, 9 Sep 2026) ───────────────────────────
## Timed status, symmetric with Vulnerable: while active, Block gained by the
## source is increased 25% (rounded up). Applied to `source`, not `target` --
## it is a self-buff like Strength/Dexterity/Faith. params: {duration}
const AGILITY = "agility"

# ── Signature cards: sequencing / conditional damage ───────────────────────────
## Deal base_amount damage; if the last card played applied a debuff to an enemy
## (StatusEffectType.is_debuff), deal bonus_amount more. Shadowfoot - Cheap Shot.
## params: {base_amount, bonus_amount}
const DAMAGE_IF_LAST_APPLIED_DEBUFF = "damage_if_last_applied_debuff"

## Deal base_amount + per_curse * (Curse cards currently in hand) damage.
## Distinct from DAMAGE_PER_CURSE (which also counts the discard pile and always
## hits all enemies) -- this one is a normal single/multi-target damage effect
## that respects the card's own targeting_mode. Witch - Hex.
## params: {base_amount, per_curse}
const DAMAGE_PER_CURSE_IN_HAND = "damage_per_curse_in_hand"

# ── Signature cards: pile-counting ──────────────────────────────────────────────
## Gain Block equal to the size of the named pile (no cap). params: {pile: "draw"|"discard"|"hand"}
## Golemancer - Bulwark.
const BLOCK_EQUAL_TO_PILE_SIZE = "block_equal_to_pile_size"

## Deal damage equal to the size of the named pile (no cap). params: {pile: "draw"|"discard"|"hand"}
## Golemancer - Construct Punch.
const DAMAGE_EQUAL_TO_PILE_SIZE = "damage_equal_to_pile_size"

# ── Signature cards: deck manipulation ──────────────────────────────────────────
## Move the top `amount` cards of the draw pile into the discard pile (mill).
## Reshuffles discard into draw when the draw pile empties, same as a normal draw.
## Grove - Deadfall. params: {amount}
const MILL_DRAW_PILE = "mill_draw_pile"

## Exhaust every Curse currently in hand (out of rotation for the rest of this
## combat -- see DeckModel.exhaust_card; the "Exhaust" card keyword uses the
## same mechanism). For each card actually exhausted, gain energy_per_card
## Energy and draw draw_per_card cards. Does nothing at 0 Curses.
## Witch - Dark Pact. params: {energy_per_card, draw_per_card}
const EXHAUST_HAND_CURSES = "exhaust_hand_curses"

# ── Party Abilities (Card-Clock Combat spec §7/§10.6) ──────────────────────────
## Delay a targeted enemy's timer (increase time_current, clamped to time_max): {amount}
const DELAY_ENEMY_TIMER = "delay_enemy_timer"

## The next card played costs `discount` less energy, and also gains Haste (0
## ticks) unless grant_haste is explicitly false: {discount, grant_haste (default true)}
## Shadowfoot's Shadow Step sets grant_haste=false (discount only, no Haste grant).
const NEXT_CARD_HASTE_AND_DISCOUNT = "next_card_haste_and_discount"

## Gain floor(discard_pile_size / per) Energy, capped at `max`: {per, max}
const ENERGY_PER_DISCARD_PILE = "energy_per_discard_pile"

## Flat Energy gain (no ceiling — Design ruling, 10 Sep 2026): {amount}
const GAIN_ENERGY = "gain_energy"

# ── Golemancer / Pet System ────────────────────────────────────────────────────
## Summon a pet defined in PetBoard's registry.
## params: { "pet_def_id": String, "hp_bonus": int (optional, default 0) }
const SUMMON_PET = "summon_pet"

## Give the oldest alive pet +max_hp and heal; optionally draw if it survives an enemy action.
## params: { "max_hp_bonus": int, "heal_amount": int, "draw_if_survives": int }
const REINFORCE_PET = "reinforce_pet"

## Power card: Whenever a Construct pet is summoned it gains +hp and heals.
## Also triggers an immediate assembly check.
## params: { "hp_bonus": int, "heal_amount": int }
const GRAND_ASSEMBLY_POWER = "grand_assembly_power"

## Deal damage to a random alive enemy at the START_OF_NEXT_PLAYER_TURN.
## params: { "amount": int }
const DELAYED_DAMAGE = "delayed_damage"

# Get all valid effect types (for validation)
static func get_all_types() -> Array[String]:
	return [
		DAMAGE,
		BLOCK,
		HEAL,
		DRAW,
		VULNERABLE,
		VULNERABLE_ALL_ENEMIES,
		STRENGTH,
		DEXTERITY,
		FAITH,
		WEAKNESS,
		GRANT_HASTE_NEXT_CARD,
		ADD_CURSE_TO_HAND,
		ADD_TEMPORARY_UPGRADE_TO_RANDOM_HAND_CARD,
		CONDITIONAL_STRENGTH_IF_NO_DAMAGE,
		DAMAGE_PER_CURSE,
		DAMAGE_CONDITIONAL_ELITE,
		BLOCK_ON_ENEMY_ACT,
		DAMAGE_ON_BLOCK_GAIN,
		DRAW_PER_TURN,
		LEGACY_KILL_COUNTER,
		APPLY_STATUS,
		MODIFY_ENEMY_TIMER,
		DAMAGE_SPITE,
		DRAW_IF_TOOK_DAMAGE,
		DAMAGE_SEQUENCING,
		BLOOM,
		REGROWTH,
		SCRY,
		DAMAGE_CONDITIONAL_TOP_CARD,
		MIRROR,
		RESONANCE_BLOCK,
		BLOCK_TO_ENERGY,
		SUMMON_PET,
		REINFORCE_PET,
		GRAND_ASSEMBLY_POWER,
		DELAYED_DAMAGE,
		DAMAGE_EQUAL_TO_BLOCK,
		DELAY_ENEMY_TIMER,
		NEXT_CARD_HASTE_AND_DISCOUNT,
		ENERGY_PER_DISCARD_PILE,
		GAIN_ENERGY,
		AGILITY,
		DAMAGE_IF_LAST_APPLIED_DEBUFF,
		DAMAGE_PER_CURSE_IN_HAND,
		BLOCK_EQUAL_TO_PILE_SIZE,
		DAMAGE_EQUAL_TO_PILE_SIZE,
		MILL_DRAW_PILE,
		EXHAUST_HAND_CURSES,
	]

# Validate if a string is a valid effect type
static func is_valid(effect_type: String) -> bool:
	return effect_type in get_all_types()

