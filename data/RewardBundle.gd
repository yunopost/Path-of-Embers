...
func _init(p_gold: int = 0, p_card_choices: Array[String] = [], p_relic_id: String = "", p_upgrade_count: int = 0, p_heal_amount: int = 0, p_skip_allowed: bool = true, p_is_transcendence: bool = false, p_upgrade_points: int = 0):
	gold = p_gold
	if p_card_choices:
...
func has_any_rewards() -> bool:
	...
	return gold > 0 or card_choices.size() > 0 or not relic_id.is_empty() or upgrade_count > 0 or heal_amount > 0 or upgrade_points > 0
