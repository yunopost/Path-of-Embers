...
@onready var stats_row: HBoxContainer = $InnerMargin/Container/StatsRow
@onready var str_label: Label = $InnerMargin/Container/StatsRow/StrLabel
@onready var def_label: Label = $InnerMargin/Container/StatsRow/DefLabel
@onready var spirit_label: Label = $InnerMargin/Container/StatsRow/SpiritLabel
@onready var hp_contribution_label: Label = $InnerMargin/Container/HPContributionLabel
...
func _show_stats(visible_now: bool) -> void:
	if is_instance_valid(stats_row):
		stats_row.visible = visible_now
	if is_instance_valid(hp_contribution_label):
		hp_contribution_label.visible = visible_now

func refresh_stats() -> void:
	## Update this character's compact stat readout (Card-Clock Addendum §4
	## item 4 / "a per-character stat readout on the battle screen"): current
	## Strength/Dexterity/Faith (base + equipment + any in-combat bonus that
	## targets this character specifically) and, since HP is a shared party
	## pool rather than per-character, how much of it this character brought
	## (base + equipped HP bonus) -- both matter for deciding which card is
	## the right play, since a character's stats only affect their own cards.
	## Call after any action that could change equipment or statuses.
	if not _combat_controller or not is_instance_valid(stats_row) or not stats_row.visible:
		return
	var char_data := DataRegistry.get_character(_character_id) if DataRegistry else null
	if not char_data:
		return

	var owner_stats: EntityStats = null
	if "character_stats" in _combat_controller:
		var cs: Dictionary = _combat_controller.character_stats
		owner_stats = cs.get(_character_id, null)

	var str_val := char_data.str_base
	var def_val := char_data.def_base
	var spirit_val := char_data.spirit_base
	if owner_stats:
	...
	if is_instance_valid(str_label):
		str_label.text = "STR %d" % str_val
	if is_instance_valid(def_label):
		def_label.text = "DEF %d" % def_val
	if is_instance_valid(spirit_label):
		spirit_label.text = "SPI %d" % spirit_val

	if is_instance_valid(hp_contribution_label):
		var hp_bonus := 0
		var equip_slots: Dictionary = RunState.equipment_slots.get(_character_id, {}) if RunState else {}
		for slot_name in equip_slots:
			...
			if equip_data:
				hp_bonus += int(equip_data.stat_modifiers.get("hp", 0))
		var total_hp := char_data.hp_base + hp_bonus
		hp_contribution_label.text = "HP +%d" % total_hp if hp_bonus == 0 else "HP +%d (%d+%d)" % [total_hp, char_data.hp_base, hp_bonus]
