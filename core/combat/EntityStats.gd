...
func add_block(amount: int):
	## Single entry point for gaining Block, from any source (cards, party
	## abilities, pet/status hooks alike). Applies the Agility bonus here, once,
	## rather than at each call site: while Agility is active, Block gained is
	## increased 25%, rounded up -- symmetric with Vulnerable's damage bonus.
	var final_amount = amount
	var agility_duration = get_status(StatusEffectType.AGILITY)
	if agility_duration != null and agility_duration > 0:
		final_amount = int(ceil(float(amount) * 1.25))
	block += final_amount
	block_changed.emit(block)

func reset_block():
	## Unconditionally reset block to 0. Block otherwise persists (shipped rule,
	## 9 Sep 2026 — Addendum C §2) and is only ever reduced by absorbing damage;
	## this setter is for effects that voluntarily zero their own block
	## (e.g. Block-to-Energy, What Was Left).
	block = 0
	block_changed.emit(block)
