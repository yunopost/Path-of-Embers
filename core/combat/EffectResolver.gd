	...
	match effect.effect_type:
		...
		EffectType.APPLY_STATUS:
			...
			if target:
				...
				_mark_debuff_applied(combat_controller, status_type)
		...
		EffectType.GRANT_HASTE_NEXT_CARD:
			...
			if RunState:
				...
				# Pursuit: "if that card costs 2 or more, draw 2" -- stash the
				# threshold/amount alongside haste_next_card; both are consumed
				# together, when the marked next card is actually played (see
				# CombatController.play_card / RunState.get_timer_tick_amount_for_card).
				var min_cost = int(effect.params.get("conditional_draw_min_cost", 0))
				var draw_amount = int(effect.params.get("conditional_draw_amount", 0))
				if min_cost > 0 and draw_amount > 0:
					RunState.next_card_conditional_draw_min_cost = min_cost
					RunState.next_card_conditional_draw_amount = draw_amount

		EffectType.VULNERABLE:
			var duration = effect.params.get("duration", 1)
			if target:
				target.apply_status(StatusEffectType.VULNERABLE, duration)
				_mark_debuff_applied(combat_controller, StatusEffectType.VULNERABLE)
		...
		EffectType.WEAKNESS:
			...
			if target:
				...
				_mark_debuff_applied(combat_controller, StatusEffectType.WEAKNESS)

		EffectType.AGILITY:
			# Self-buff, symmetric with Vulnerable: applies to `source` (the card's
			# owner), not `target` (an enemy) -- same pattern as STRENGTH/DEXTERITY/FAITH.
			var duration = effect.params.get("duration", 1)
			if source:
				source.apply_status(StatusEffectType.AGILITY, duration)
		...
		EffectType.BLOCK_TO_ENERGY:
			...
			if source and combat_controller and block_per_energy > 0:
				...
				if current_block_val > 0:
					...
					if energy_gained > 0:
						# Energy has no ceiling (Design ruling, 10 Sep 2026) — no max_e clamp.
						var cur_e = int(combat_controller.get("current_energy") if combat_controller.get("current_energy") != null else 0)
						var new_energy = cur_e + energy_gained
						combat_controller.set("current_energy", new_energy)
						if ResourceManager:
							ResourceManager.set_energy(new_energy)
					# TODO: RunState.increment_quest_counter("block_converted_to_energy", current_block_val)

		EffectType.DAMAGE_EQUAL_TO_BLOCK:
			# Deal damage equal to the player's current block value, divided by
			# `divisor` (default 1) and rounded up. Iron Tide passes divisor=2
			# ("half your current Block"). Never spends Block (source.block is
			# read-only here).
			var block_divisor = int(effect.params.get("divisor", 1))
			if block_divisor < 1:
				block_divisor = 1
			var block_value = 0
			if source:
				block_value = int(ceil(float(source.block) / float(block_divisor)))
		...
		EffectType.NEXT_CARD_HASTE_AND_DISCOUNT:
			## The next card played gets a cost discount (via RunState.next_card_discount,
			## consumed by RunState.get_timer_tick_amount_for_card) and, unless
			## grant_haste is explicitly false, also gains Haste (0 ticks, via
			## RunState.haste_next_card, consumed alongside it). Shadow Step sets
			## grant_haste=false -- discount only, no Haste grant.
			var discount_amount: int = int(effect.params.get("discount", 1))
			var grant_haste: bool = bool(effect.params.get("grant_haste", true))
			if RunState:
				if grant_haste:
					RunState.haste_next_card = true
		...
		EffectType.ENERGY_PER_DISCARD_PILE:
			## Gain floor(discard_pile_size / per) Energy, capped at `max` (a per-effect
			## balance cap on this specific gain, unrelated to the energy resource itself,
			## which has no ceiling — Design ruling, 10 Sep 2026).
			...
			if per_cards > 0 and RunState and RunState.deck_model and combat_controller:
				...
				if gained > 0:
					...
					var new_e: int = cur_e + gained
					combat_controller.set("current_energy", new_e)
					if ResourceManager:
						ResourceManager.set_energy(new_e)

		EffectType.GAIN_ENERGY:
			## Flat Energy gain. Energy has no ceiling (Design ruling, 10 Sep 2026).
			...
			if combat_controller and energy_amount > 0:
				...
				var new_e2: int = cur_e2 + energy_amount
				combat_controller.set("current_energy", new_e2)
				if ResourceManager:
					ResourceManager.set_energy(new_e2)
		...
		# ── Signature cards (9 Sep 2026) ─────────────────────────────────────────
		EffectType.DAMAGE_IF_LAST_APPLIED_DEBUFF:
			## Shadowfoot - Cheap Shot: base damage, plus bonus damage if the
			## LAST card played applied a debuff to an enemy (CombatController.last_card_applied_debuff).
			var base_dild = int(effect.params.get("base_amount", 0))
			var bonus_dild = int(effect.params.get("bonus_amount", 0))
			var last_applied_debuff = false
			if combat_controller:
				last_applied_debuff = bool(combat_controller.get("last_card_applied_debuff"))
			var total_dild = base_dild + (bonus_dild if last_applied_debuff else 0)

			var strength_dild = 0
			if source:
				var sv = source.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					strength_dild = int(sv)
			var owner_str_dild = 0
			if owner_stats:
				var sv = owner_stats.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					owner_str_dild = int(sv)
			var weakness_dild = false
			if source:
				var wv = source.get_status(StatusEffectType.WEAKNESS)
				if wv != null and int(wv) > 0:
					weakness_dild = true
			var damage_bw_dild = total_dild + strength_dild + owner_str_dild
			var final_dild = damage_bw_dild
			if weakness_dild:
				final_dild = int(ceil(float(damage_bw_dild) * 0.75))
			if target:
				target.take_damage(final_dild, false)

		EffectType.DAMAGE_PER_CURSE_IN_HAND:
			## Witch - Hex: base + per_curse * (Curse cards currently in hand).
			## Unlike DAMAGE_PER_CURSE, this is a normal single/multi-target damage
			## effect -- it respects the card's own targeting_mode instead of always
			## hitting every enemy -- and only counts the hand, not the discard pile.
			var base_dpch = int(effect.params.get("base_amount", 0))
			var per_curse_dpch = int(effect.params.get("per_curse", 0))
			var curse_count_dpch = 0
			if RunState and RunState.deck_model:
				for instance_id in RunState.deck_model.hand:
					var card = RunState.deck.get(instance_id)
					if card:
						...
						if cd and cd.card_type == CardData.CardType.CURSE:
							curse_count_dpch += 1
			var total_dpch = base_dpch + per_curse_dpch * curse_count_dpch

			var strength_dpch = 0
			if source:
				var sv = source.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					strength_dpch = int(sv)
			var owner_str_dpch = 0
			if owner_stats:
				var sv = owner_stats.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					owner_str_dpch = int(sv)
			var weakness_dpch = false
			if source:
				var wv = source.get_status(StatusEffectType.WEAKNESS)
				if wv != null and int(wv) > 0:
					weakness_dpch = true
			var damage_bw_dpch = total_dpch + strength_dpch + owner_str_dpch
			var final_dpch = damage_bw_dpch
			if weakness_dpch:
				final_dpch = int(ceil(float(damage_bw_dpch) * 0.75))
			if target:
				target.take_damage(final_dpch, false)

		EffectType.BLOCK_EQUAL_TO_PILE_SIZE:
			## Golemancer - Bulwark: Block equal to the size of the named pile.
			## No cap -- deliberate ruling, do not add one.
			var pile_name_b = str(effect.params.get("pile", "discard"))
			var pile_size_b = _pile_size(pile_name_b)
			var dexterity_pile = 0
			if source:
				var dv = source.get_status(StatusEffectType.DEXTERITY)
				if dv != null:
					dexterity_pile = int(dv)
			var owner_dex_pile = 0
			if owner_stats:
				var dv = owner_stats.get_status(StatusEffectType.DEXTERITY)
				if dv != null:
					owner_dex_pile = int(dv)
			if source:
				source.add_block(pile_size_b + dexterity_pile + owner_dex_pile)

		EffectType.DAMAGE_EQUAL_TO_PILE_SIZE:
			## Golemancer - Construct Punch: damage equal to the size of the named
			## pile. No cap -- deliberate ruling, do not add one.
			var pile_name_d = str(effect.params.get("pile", "draw"))
			var pile_size_d = _pile_size(pile_name_d)

			var strength_pile = 0
			if source:
				var sv = source.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					strength_pile = int(sv)
			var owner_str_pile = 0
			if owner_stats:
				var sv = owner_stats.get_status(StatusEffectType.STRENGTH)
				if sv != null:
					owner_str_pile = int(sv)
			var weakness_pile = false
			if source:
				var wv = source.get_status(StatusEffectType.WEAKNESS)
				if wv != null and int(wv) > 0:
					weakness_pile = true
			var damage_bw_pile = pile_size_d + strength_pile + owner_str_pile
			var final_pile = damage_bw_pile
			if weakness_pile:
				final_pile = int(ceil(float(damage_bw_pile) * 0.75))
			if target:
				target.take_damage(final_pile, false)

		EffectType.MILL_DRAW_PILE:
			## Grove - Deadfall: move the top `amount` cards of the draw pile into
			## the discard pile. Reshuffles discard into draw when it empties, same
			## rule as a normal draw.
			var mill_amount = int(effect.params.get("amount", 1))
			if RunState and mill_amount > 0:
				RunState.mill_draw_pile(mill_amount)

		EffectType.EXHAUST_HAND_CURSES:
			## Witch - Dark Pact: exhaust every Curse in hand; per card actually
			## exhausted, gain energy and draw. Does nothing at 0 Curses -- handled
			## in resolve_effects (returns the draw count, like DRAW).
			var energy_per_card = int(effect.params.get("energy_per_card", 1))
			var draw_per_card = int(effect.params.get("draw_per_card", 1))
			var curse_ids: Array[String] = []
			if RunState and RunState.deck_model:
				for instance_id in RunState.deck_model.hand.duplicate():
					var card = RunState.deck.get(instance_id)
					if card:
						var cd = DataRegistry.get_card_data(card.card_id)
						if cd and cd.card_type == CardData.CardType.CURSE:
							curse_ids.append(instance_id)
			var exhausted_count = curse_ids.size()
			if exhausted_count > 0:
				for instance_id in curse_ids:
					RunState.exhaust_card_instance(instance_id)
				if energy_per_card > 0 and combat_controller:
					# Energy has no ceiling (Design ruling, 10 Sep 2026) — no max_e clamp.
					var cur_e3: int = int(combat_controller.get("current_energy"))
					var new_e3: int = cur_e3 + energy_per_card * exhausted_count
					combat_controller.set("current_energy", new_e3)
					if ResourceManager:
						ResourceManager.set_energy(new_e3)
				if draw_per_card > 0:
					return draw_per_card * exhausted_count

		_:
			push_warning("Unknown effect type: " + effect.effect_type)

static func _mark_debuff_applied(combat_controller: Node, status_type: String) -> void:
	## Flip CombatController._current_card_applied_debuff when a debuff-classified
	## status (StatusEffectType.is_debuff) lands on an enemy, so the NEXT card
	## played (Cheap Shot) can read last_card_applied_debuff. No-op if
	## combat_controller is absent (e.g. a unit test resolving effects directly)
	## or status_type isn't classified as a debuff.
	if combat_controller and StatusEffectType.is_debuff(status_type):
		combat_controller.set("_current_card_applied_debuff", true)

static func _pile_size(pile_name: String) -> int:
	## Resolve a "which pile" effect param (Bulwark/Construct Punch) to its
	## current size. Unknown pile names count as 0 rather than erroring.
	...
	match pile_name:
		...
		_:
			push_warning("EffectResolver._pile_size: unknown pile '%s'" % pile_name)
			return 0
...
static func resolve_effects(effects: Array, source: EntityStats, target: EntityStats = null, enemy_context: Enemy = null, combat_controller: Node = null, owner_stats: EntityStats = null) -> int:
	...
	for effect in effects:
		if effect is EffectData:
			if effect.effect_type == EffectType.DRAW or effect.effect_type == EffectType.DRAW_IF_TOOK_DAMAGE or effect.effect_type == EffectType.EXHAUST_HAND_CURSES:
