...
## Anti-turtle damage escalation (EnemyData.escalation_*): how many Strength stacks
## this enemy has already been granted, so CombatController.advance_clock only ever
## applies the delta as the fight's tick clock passes each interval.
var escalation_stacks_applied: int = 0
