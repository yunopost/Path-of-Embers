...
**Who wrote it:** the game's art director (Claude, in the design project). Everything here follows the project's Style Bible and World Bible. Do not invent new style rules; where this file is silent, match the existing finished art in `Path-of-Embers/Art Assets/`.
...
- `Path-of-Embers/Art Assets/Monster Hunter/Monster Hunter.png` — the line and paint register for anything with a face
- `Path-of-Embers/Art Assets/Witch/Witch 2.png` — same
- `Path-of-Embers/Art Assets/Enemy Sprites/Ashen Knight (elite).png` — the enemy register
- `Path-of-Embers/Art Assets/Card Assets/Combat background.png` — the environment register
- `Path-of-Embers/Art Assets/Start Menu/Main Menu background v2.png` — the ink-wash register for menus
...
`Path-of-Embers/Art Assets/_reference/blockouts/` contains crude grey-rectangle layouts. They are **composition references only**: keep the arrangement, camera height, and where the sun and the big foreground shape are. Ignore their colours and crudeness entirely.
...
| # | Source | Edit | Output |
|---|---|---|---|
| E1 | `Path-of-Embers/Art Assets/Shadowfoot/Shadowfoot.png` | Remove the painted checkerboard behind the figure → real transparency. Recolour the purple scarf to dark ash-grey / deep slate (`#2A3040`), keep everything else. | `Path-of-Embers/Art Assets/Shadowfoot/shadowfoot_clean.png` (same size) |
| E2 | `Path-of-Embers/Art Assets/Witch/Witch 2.png` | Remove the white background → real transparency, feathered hair edge. No other change. | `Path-of-Embers/Art Assets/Witch/witch_clean.png` |
| E3 | `Path-of-Embers/Art Assets/Enemy Sprites/Cinder Imp.png` | Remove the red/green/yellow colour fringe around the ember glows and edges; keep the transparency clean. No other change. | `.../Enemy Sprites/cinder_imp_clean.png` |
| E4 | `Path-of-Embers/Art Assets/Enemy Sprites/Smolder Shade.png` | Same fringe cleanup on the smoke wisps. | `.../Enemy Sprites/smolder_shade_clean.png` |
| E5 | `Path-of-Embers/Art Assets/Enemy Sprites/Char Sentinel (elite).png` | Same fringe cleanup along the lower edge. | `.../Enemy Sprites/char_sentinel_clean.png` |
| E6 | `Path-of-Embers/Art Assets/Boss Sprites/Fallen Obsidian Colossus.png` | Same fringe cleanup. | `.../Boss Sprites/obsidian_colossus_clean.png` |
| E7 | `Path-of-Embers/Art Assets/Boss Sprites/The Eternal Ember.png` | This image is being **reassigned as the final boss, the Lich**. Remove the yellow halo fringe around the glow; keep the figure and its glow. | `.../Boss Sprites/the_lich.png` |
| E8 | `Path-of-Embers/Art Assets/Card Assets/Raw/Card Back 1.png` (848×1264, 2:3) | Re-proportion to **3:4** by extending the design vertically-cropped/horizontally-extended as needed so nothing is squashed: output 900×1200. Keep the ember heart centred. Remove the white matte outside the card → transparent corners. | `.../Card Assets/card_back.png` (900×1200) |
...
A wide cinematic illustration of the character from the waist up, standing in the named place, placed off-centre (left or right third, never centred). The figure is the only warm, living thing; the world behind recedes into ashen blue-grey and cold teal haze. Golden light from the low left. **Use the character's existing portrait as the face/costume reference** (paths in §1 and `Art Assets/<Name>/`). Nothing behind the figure moves.

| # | Character (reference file) | Place | The one frozen detail | Output |
|---|---|---|---|---|
| S1 | Monster Hunter (`Monster Hunter/Monster Hunter.png`) | a hillside track above a frozen town at dusk | a flock of birds hanging motionless mid-flight | `Path-of-Embers/Art Assets/Splash/monster_hunter_splash.png` |
...
→ `Path-of-Embers/Art Assets/Boss Sprites/the_eternal_ember.png`
...
→ all into `Path-of-Embers/Art Assets/Card Assets/`
...
Into `Path-of-Embers/Art Assets/UI/`:
